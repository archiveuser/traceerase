(() => {
  ...
  const ru = {
    ...
    'report.waiting': '// ожидание данных…', 'report.savePdf': 'Печать / PDF', 'report.pdfHint': 'В окне печати выбери «Сохранить как PDF»',
    ...
    'graph.kicker': 'Карта следа', 'graph.title': 'Связи появляются по мере проверки', 'graph.count': 'узлов',
    ...
    'graph.rootTitle': 'Исходный идентификатор: {target}', 'graph.profile': 'публичный профиль',
    'graph.legend': 'Центр — введённый идентификатор. Узлы появляются только после подтверждённого публичного ответа.',
  ...
  const en = {
    ...
    'graph.kicker': 'Trace map', 'graph.title': 'Connections appear as the check runs', 'graph.count': 'nodes',
    ...
    'graph.rootTitle': 'Source identifier: {target}', 'graph.profile': 'public profile',
    'graph.legend': 'The centre is the entered identifier. Nodes appear only after a confirmed public response.',
